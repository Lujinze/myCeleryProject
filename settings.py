...
CELERY_RESULT_BACKEND = "redis://127.0.0.1:6379/1"  # 任务结果存在Redis
...
CELERY_TASK_DEFAULT_EXCHANGE = "tasks"  # 默认的交换机名字为tasks
CELERY_TASK_DEFAULT_EXCHANGE_TYPE = "topic"  # 默认的交换类型是topic
CELERY_TASK_DEFAULT_ROUTING_KEY = "task.default" # 默认的路由键是task.default，这个路由键符合上面的default队列

CELERY_ROUTES = (
    [
        (
            re.compile(r"myCeleryProject\.tasks\.(taskA|taskB)"),
            {"queue": "tasks_A", "routing_key": "A.import"},
        ),  # 将tasks模块中的taskA,taskB分配至队列 tasks_A ,支持正则表达式
        (
            "myCeleryProject.tasks.add",
